...
# Import sqrt host for the hypot operation
import sys
...
def fused_square_add_then_sqrt(input_tensor1: ttnn.Tensor, input_tensor2: ttnn.Tensor) -> ttnn.Tensor:
    ...
    This is the partial fused version: square(A) + square(B) in one kernel, then sqrt separately.
    """
...
def run():
    ...
    # Test the fused square+add kernel
    res = fused_square_add(c, d)
    res_torch = ttnn.to_torch(res, device=device)
    
    # Expected result: square(a) + square(b)
    gold = torch.square(a) + torch.square(b)

    print("Testing fused_square_add kernel (square+add only):")
